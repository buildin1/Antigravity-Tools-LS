...
const Monitor = () => {
  ...
  const renderTrafficItem = (log) => {
    ...
    }[log.protocol] || 'text-foreground/40';
    ...
    return (
      ...
      >
        ...
        <div className="w-16 shrink-0 font-mono text-[10px] text-foreground/20">
          {new Date(log.timestamp).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })}
        </div>
        <div className={`w-12 shrink-0 font-bold text-xs ${isError ? 'text-red-500' : 'text-emerald-500'}`}>{log.status}</div>
        <div className="w-16 shrink-0 font-black text-[10px] uppercase tracking-tighter text-foreground/40">{log.method}</div>
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2">
          ...
          <span className="text-foreground/40 text-[10px] font-mono truncate tracking-tight italic">{log.mapped_model || 'none'}</span>
        </div>
        <div className="w-20 shrink-0 text-right font-mono text-[10px] text-foreground/40">{log.duration}ms</div>
      </div>
    );
  };
  ...
  const renderSysLogItem = (entry) => {
    ...
    }[entry.level] || 'text-foreground/40';
    ...
    return (
      <div key={entry.id} className="flex gap-4 px-4 py-1.5 border-b border-glass-border font-mono text-[11px] leading-relaxed group hover:bg-foreground/[0.01]">
        <div className="w-20 shrink-0 text-foreground/20 tracking-tighter">
        ...
        <div className="w-32 shrink-0 text-foreground/30 truncate italic">[{entry.target}]</div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className={`p-2.5 rounded-xl transition-all ${isPaused ? 'btn-matrix-pill-active !text-amber-500 !bg-amber-500/10 !border-amber-500/20' : 'btn-matrix-glass text-foreground/40'}`}
          >
          ...
          <button 
            ...
            className="p-2.5 rounded-xl btn-matrix-glass text-foreground/40 hover:!text-red-500 hover:!border-red-500/20"
          >
          ...
          <div className="px-4 py-3 bg-foreground/[0.03] border-b border-glass-border flex items-center justify-between shrink-0">
            ...
            <div className="flex border border-glass-border rounded-lg overflow-hidden shrink-0">
              ...
              <button 
                ...
                className={`px-3 py-1 text-[10px] font-bold uppercase border-l border-glass-border transition-all ${filterLevel === 'error' ? (activeTab === 'traffic' ? 'bg-red-500/20 text-red-500' : 'bg-red-500/30 text-red-400') : 'text-foreground/20 hover:text-foreground/40'}`}
              >
        ...
        {activeTab === 'traffic' && selectedLog && (
          <div className="w-[450px] bg-foreground/[0.02] border border-glass-border rounded-2xl flex flex-col overflow-hidden animate-in slide-in-from-right-4 duration-300 shadow-2xl backdrop-blur-3xl spotlight-card relative">
            ...
            <div className="p-6 border-b border-glass-border flex items-center justify-between shrink-0">
              ...
              <button onClick={() => setSelectedLog(null)} className="text-foreground/20 hover:text-foreground transition-all text-sm uppercase font-black tracking-tighter">{t('common.close')}</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide font-mono">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-foreground/[0.03] border border-glass-border">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase text-foreground/20 mb-2">
              ...
              <div className="p-4 rounded-xl bg-background/40 border border-glass-border">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col gap-1">
                    <span className="text-[9px] text-foreground/30 uppercase font-black px-1">{t('monitor.original')}</span>
              ...
              {(selectedLog.input_tokens > 0 || selectedLog.output_tokens > 0) && (
                <div className="p-4 rounded-xl bg-foreground/5 border border-glass-border">
                  <div className="text-[10px] font-black uppercase text-foreground/20 mb-4 tracking-widest">{t('monitor.tokenUsage')}</div>
...
const EmptyState = ({ icon: Icon, label }) => (
  <div className="h-full flex flex-col items-center justify-center gap-4 text-foreground/10">
